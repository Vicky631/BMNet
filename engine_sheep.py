...
EVAL_CFG = {
    ...
    "annotation_path": '/mnt/mydisk/wjj/dataset/Sheep_obb/DOTA',
    "image_root_path": '/mnt/mydisk/wjj/dataset/Sheep_obb/img',
    "csv_save_dir": "/mnt/mydisk/wjj/BMNet/experiments/Sheep_obb/eval_csv"  # CSV保存目录
}
# EVAL_CFG = {
#     "distance_thresh": 10.0,  # 定位匹配距离阈值
#     "dsizensity_map_size": (512, 512),  # 密度图固定尺寸
...
#     # 获取Sheep_obb数据的标注信息
#
#     sheep_loader = SheepOBBDatasetLoader(
#         annotation_dir=EVAL_CFG["annotation_path"],
#         image_dir=EVAL_CFG["image_root_path"],
...
#     )
#     all_filenames = sheep_loader.get_all_filenames()
#     # for idx, sample in enumerate(data_loader):
#     for fname in tqdm(all_filenames, desc="批量推理"):
#         # img, patches, targets = sample
#         result = sheep_loader.get_annotations(image_name, return_scaled=True, return_numpy=False)
#         gt_points = result['centers']
#         gtcount = len(result['centers'])
#         img = result['img']
#
#         img = img.to(device)
#         patches = {}
#         patches['patches'] = result['patches'].to(device)
#         patches['scale_embedding'] = result['scales'].to(device)
#
...
# 假设你已定义以下依赖（若未定义需补充）
# from dataset.sheep_obb import SheepOBBDatasetLoader  # 羊数据集加载器
# from utils.csv_writer import EvaluationCSVWriter    # CSV写入器
# from config import EVAL_CFG                          # 评估配置

@torch.no_grad()
def evaluate(model, data_loader, device, output_dir):
    """
    修正版羊数据集评估函数（适配SheepOBBDatasetLoader）
    :param model: 待评估的计数模型
    ...
    os.makedirs(EVAL_CFG["csv_save_dir"], exist_ok=True)
    ...
    # ==================批量推理与指标计算======================
    with torch.no_grad():  # 评估阶段禁用梯度计算，节省显存
        for idx, fname in enumerate(tqdm(all_filenames, desc="批量推理")):
            # --------------------------
            # 1. 获取单张图片的所有数据
            # --------------------------
            data = sheep_loader[idx]
            img_pil = data['image']          # PIL图像
            gt_points = data['centers']      # 真实中心点 (N,2) numpy数组
            gtcount = len(gt_points)         # 真实计数（整数）
            patches_tensor = data['patches'] # patch Tensor (N,3,128,128)
            scales_tensor = data['scales']   # scale Tensor (N,)

            # --------------------------
            # 2. 数据预处理并转到GPU
            # --------------------------
            # 图像预处理 + 增加batch维度
            img = img_transform(img_pil).unsqueeze(0).to(device)  # (1,3,H,W)

            # 处理空patch情况（避免模型报错）
            if len(patches_tensor) == 0:
                patches_tensor = torch.zeros((0, 3, 128, 128))
                scales_tensor = torch.zeros(0, dtype=torch.long)

            # 构造patches字典 + 增加batch维度 + 转到GPU
            patches = {
                'patches': patches_tensor.unsqueeze(0).to(device),  # (1,N,3,128,128)
                'scale_embedding': scales_tensor.unsqueeze(0).to(device)  # (1,N)
            }

            # --------------------------
            # 3. 模型推理
            # --------------------------
            outputs = model(img, patches, is_train=False)  # 输出密度图 (1,1,H,W)

            # --------------------------
            # 4. 定位指标计算（Precision/Recall/F1）
            # --------------------------
            precision = 0.0
            recall = 0.0
            f1 = 0.0
            ...
            try:
                ...
                print(f"警告：计算{fname}定位指标失败: {str(e)}")

            # --------------------------
            # 5. 计数指标计算（MAE/MSE）
            # --------------------------
            pred_count = outputs.sum().item()  # 密度图求和得到预测计数
            error = abs(pred_count - gtcount)
            mae += error
            mse += error ** 2
            ...
            # 7. 打印单样本指标（实时监控）
            # --------------------------
            print(f"[{idx+1}/{total_samples}] {fname} - "
                  f"GT:{gtcount}, Pred:{pred_count:.2f}, "
                  f"Precision:{precision:.4f}, Recall:{recall:.4f}, F1:{f1:.4f}")
    ...
    # 1. 样本级指标
    sample_metrics = {
        "filenames": all_filenames,          # 初始化好的文件名列表（无重复）
        "gt_counts": all_gt_counts,
        "pred_counts": all_pred_counts,
        "precisions": all_precisions,
        "recalls": all_recalls,
    ...
    # 2. 汇总级指标
    summary_metrics = {
        "dataset_type": "sheep_obb_evaluation",
        "total_samples": total_samples,
        "distance_thresh": EVAL_CFG["distance_thresh"],
        "avg_mae": avg_mae,
        "avg_rmse": avg_mse,
    ...
    # ==================保存到日志文件======================
    log_content = (
        f"Sheep OBB Dataset Evaluation Results:\n"
        f"Total Samples: {total_samples}\n"
        f"MAE {avg_mae:.2f}, RMSE {avg_mse:.2f} \n"
        f"Average Precision {avg_precision:.4f} (±{std_precision:.4f}), "
        f"Average Recall {avg_recall:.4f} (±{std_recall:.4f}), "
        f"Average F1 {avg_f1:.4f} (±{std_f1:.4f})\n"
        f"----------------------------------------\n"
