...
os.environ['TORCH_HOME'] = '/mnt/mydisk/wjj/online_models/torch_cache'
import numpy as np
import torch
...
def main(args):
    ...
    data_loader_train = DataLoader(dataset_train, batch_size=cfg.TRAIN.batch_size, collate_fn=batch_collate_fn,
    ...
    for epoch in range(cfg.TRAIN.start_epoch, cfg.TRAIN.epochs):
        ...
        if cfg.DIR.output_dir:
            with (output_dir / "log.txt").open("a") as f:
                f.write(
                    'Epoch %d: loss %.8f, MAE %.2f, MSE %.2f, Best MAE %.2f, Best MSE %.2f \n' % (epoch + 1, loss, mae,
                                                                                                  mse, best_mae,
                                                                                                  best_mse))
